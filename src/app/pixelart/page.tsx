...
export default function PixelArt() {
  ...
  function getTextColor() {
    ...
    // Converta o hexadecimal para números inteiros
      ...
      <div className="flex items-center justify-center w-screen h-screen">
        <div className="grid grid-cols-1 gap-2 min-w-[150px]">
          <button
            ...
            className="text-white rounded-lg border border-green-700 bg-green-500 hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-300 transition duration-200 px-4 py-2"
          >
            Salvar
          </button>
        ...
        <div className="flex flex-col items-center justify-start bg-gray-200 rounded-lg shadow-md border-2 p-3">
          ...
          <HexColorInput color={selectedColor} onChange={setSelectedColor} placeholder="Type a color" prefixed alpha className={`max-w-[100px] mt-2 rounded-lg border-1 shadow-md shadow-gray-500 border-black text-center ${getTextColor()}`} style={{ backgroundColor: selectedColor, }} />
          <div className="flex flex-col justify-center items-center align-middle gap-2 mt-5 mb-5">
            <Image
              src="/contagotas.svg"
              alt="Selecionar Cor"
              onClick={startPickingColor}
              className={`w-10 h-10 flexcursor-pointer shadow-lg shadow-gray-500 p-1 rounded-lg transition-transform duration-200 ${pickingColor ? 'scale-110 bg-gray-500 rounded-full' : 'scale-100'}`}
              width={0}
              height={0}
            />
            <button
              onClick={() => { handleBorder(border); }}
              className="text-white rounded-lg border border-gray-700 bg-blue-500 hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300 transition duration-200 px-4 py-2 mt-5 w-[100px]"
            >
              {border ? "Grid" : "Grid"}
            </button>
            {/* <div
